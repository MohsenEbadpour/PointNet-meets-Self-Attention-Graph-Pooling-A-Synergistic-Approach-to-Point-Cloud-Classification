...
def TestPerfomancePointNet(model,loader):
    with torch.no_grad():
        ...
        loss = 0.
        for data in loader:
            inputs, labels = data['graph_features'][:, :, [0, 1, 2, 7]].to("cuda:0").float(), data['category'].to("cuda:0")
...
def TrainPointNet(model, train_loader, val_loader,lr=0.01,weight_decay=0.0005, epochs=30, name="PointNet"):
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(train_loader):
            inputs, labels = data['graph_features'][:, :, [0, 1, 2, 7]].to(device).float(), data['category'].to(device)
            optimizer.zero_grad()
    ...
    plt.tight_layout()
    plt.savefig("./results/pointnet/point-cloud/{0}_{1}_{2}_{3}_harmonic_centrality.png".format(name, "ModelNet10", lr, weight_decay))
    plt.show()
    plt.clf()
...
# Save mode weights
torch.save(model.state_dict(), "../outputs/model_weights/ModelNet10_{0}_{1}_{2}_harmonic_centrality".format(epoch, learning_rate, weight_decay))

# Save best validation loss and the corresponding model weights
torch.save(best_model.state_dict(), "../outputs/best_records/ModelNet10_{0}_{1}_{2}_harmonic_centrality".format(epoch, learning_rate, weight_decay))

with open("../outputs/best_records/ModelNet10_{0}_{1}_{2}_harmonic_centrality.txt".format(epoch, learning_rate, weight_decay), "w") as f:
    f.write(str(best_val_loss))

with open("../outputs/accuracy_log/ModelNet10_validation_loss_{0}_{1}_{2}_harmonic_centrality.txt".format(epoch, learning_rate, weight_decay), 'w') as f:
    for item in val_loss:
        f.write("%s\n" % item)

with open("../outputs/accuracy_log/ModelNet10_train_loss_{0}_{1}_{2}_harmonic_centrality.txt".format(epoch, learning_rate, weight_decay), 'w') as f:
    for item in train_loss:
        f.write("%s\n" % item)

with open("../outputs/accuracy_log/ModelNet10_validation_acc_{0}_{1}_{2}_harmonic_centrality.txt".format(epoch, learning_rate, weight_decay), "w") as f:
    for item in val_acc:
        f.write("%s\n" % item)

with open("../outputs/accuracy_log/ModelNet10_train_acc_{0}_{1}_{2}_harmonic_centrality.txt".format(epoch, learning_rate, weight_decay), "w") as f:
